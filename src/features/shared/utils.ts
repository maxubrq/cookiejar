export function toOriginPermissionPattern(
    input: string,
    variant?: 'sub' | 'dot',
): string | null {
    try {
...
export async function requestDomainCookieAccess(
 ...
 */
export function isDomainCoveredByOriginPattern(
    originPattern: string,
    domain: string,
): boolean {
    try {
        ...
        const normalizedDomain = new URL(
